...
def compute_correlogram_normalized_cc_first(
    x, y, max_lag_ms, bin_size, mode, firing_rate_x, firing_rate_y, T, score_type="bump"
):
    ...
    def bin_correlate(x, y, max_lag_ms, bin_size, T, mode):
        ...
        binned = np.array(
            [
                corr_trim[
                    int(lags[i] + max_lag_ms) : int(lags[i + 1] + max_lag_ms)
                ].sum()
...
def plot_neuron_autocorrelations_combined(neurons, save_dir, sample_rate):
    ...
    for page in range(n_pages):
        page_neurons = neuron_ids[
            page
            * neurons_per_page : min((page + 1) * neurons_per_page, len(neuron_ids))
        ]
        ...
        fig.suptitle(
            f'Autocorrelation of Neuron Spike Trains at Multiple Resolutions{" (Page " + str(page+1) + "/" + str(n_pages) + ")" if n_pages > 1 else ""}',
            fontsize=10,
            y=0.98,
        )
        plt.tight_layout()
        ...
        plt.savefig(
            os.path.join(
                save_dir, f"neuron_autocorrelations_combined_page{page+1}.png"
            ),
            dpi=150,
        )
        print(
            f'Saved at {os.path.join(save_dir, f"neuron_autocorrelations_combined_page{page+1}.png")}'
        )
        plt.close()
...
def plot_neuron_correlation_matrices(
    neurons, save_dir, sample_rate=1, edge_mean=True, configs=None, make_plots=True
):
    if configs is None:
        configs = [
            (80, 1000, "Broad"),
    ...
    for bin_size, max_lag, resolution in configs:
        ...
        autocorrs = {
            (pre): compute_correlogram_normalized_cc_first(
                neuron_spike_trains[pre].astype(float),
                neuron_spike_trains[pre].astype(float),
                max_lag,
                bin_size,
        ...
        if make_plots:
            ...
            for idx, neuron_id in enumerate(neuron_ids):
                ...
                ax_top.fill_between(
                    lags, 0, corr, color="blue", alpha=1, step="mid", linewidth=0
                )
            ...
            with open(
                os.path.join(
                    save_dir,
                    f"autocorrs_edge_mean_{edge_mean}_{resolution.lower()}.pkl",
                ),
                "wb",
            ) as f:
                pickle.dump(autocorrs, f)
            print(
                f"Saved {resolution} resolution crosscorrs and autocorrs at {save_dir}"
            )
